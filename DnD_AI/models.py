...
class Weapon(models.Model):
    ...
    weapon_type = models.CharField(
        max_length=15, default="Body part"
    )  # type was reserved
    damage_type = models.CharField(max_length=15, default="Physical")

    physical_description = models.CharField(
        max_length=100, default="It's using his own hands"
    )
    image = models.ImageField(
        upload_to="weapon/images/", default="weapon/images/default/bare_hands.png"
    )

    # Statistics
    damage = models.IntegerField(default=6)
    range = models.IntegerField(default=1)
    range_level_points = models.IntegerField(
        default=0
    )  # only for ranged weapons. 3 range level ups -> range +1
...
class Entity(models.Model):
    ...
    weapon = models.ForeignKey(
        Weapon, on_delete=models.SET(get_bare_hands), null=True, blank=True
    )
    ...
    def is_in_range(self, target):
        return (
            True
            if abs(self.x - target.x) <= self.weapon.range
            and abs(self.y - target.y) <= self.weapon.range
            else False
        )

    def get_default_entity_icon(
        self, entity_race: str, entity_class: str = "Warrior"
    ) -> str:
        if entity_race == "Human":
            if entity_class in DEFAULT_WEAPON_PER_CLASS.keys():
                return (
                    f"entity/icons/default/{entity_class.lower().replace(' ', '_')}.png"
                )
            else:
            ...
            if entity_race in DEFAULT_WEAPON_PER_CLASS.keys():
                return (
                    f"entity/icons/default/{entity_race.lower().replace(' ', '_')}.png"
                )
            else:
                return "entity/images/default.png"
    ...
    def move(self, direction: str) -> bool:
        ...
        if (
            Tile.objects.filter(campaign=self.campaign.id, x=pos[0], y=pos[1]).exists()
            and not Character.objects.filter(
                campaign=self.campaign.id, x=pos[0], y=pos[1]
            ).exists()
            and not Monster.objects.filter(
                campaign=self.campaign.id, x=pos[0], y=pos[1]
            ).exists()
            and not Treasure.objects.filter(
                campaign=self.campaign.id, x=pos[0], y=pos[1]
            ).exists()
        ):
            self.x = pos[0]
...
class Character(Entity, models.Model):
    ...
    def get_monsters_in_range(self, name: str = None):
        ...
        monsters = (
            Monster.objects.filter(campaign_id=self.campaign.id)
            if name is None
            else Monster.objects.filter(
                campaign_id=self.campaign.id, name__icontains=name
            )
        )
        for monster in monsters:
            if (
                abs(self.x - monster.x) <= self.weapon.range
                and abs(self.y - monster.y) <= self.weapon.range
            ):
                monsters_in_range.append(monster)
        return monsters_in_range

    def get_treasures_in_range(
        self, treasure_to_take: str = "all", is_weapon: bool = False
    ):
        treasures_in_range = []
        if not is_weapon:
            for treasure in Treasure.objects.filter(campaign_id=self.campaign.id):
                if (
                    abs(self.x - treasure.x) <= 1
                    and abs(self.y - treasure.y) <= 1
                    and (
                        treasure_to_take == "all"
                        or treasure_to_take == treasure.treasure_type.lower()
                    )
                ):
                    treasures_in_range.append(treasure)
        else:
            for treasure in Treasure.objects.filter(
                campaign_id=self.campaign.id, treasure_type="Weapon"
            ):
                if (
                    abs(self.x - treasure.x) <= 1
                    and abs(self.y - treasure.y) <= 1
                    and treasure.weapon.name.lower() == treasure_to_take
                ):
                    treasures_in_range.append(treasure)
    ...
    def level_up_stat(self, stat: str = "max_health"):
        ...
        History.objects.create(
            campaign=self.campaign,
            author="SYSTEM",
            text=f"{self.name} leveled up his {stat}!",
        ).save()
    ...
    def level_up_weapon(self, stat: str = "damage", new_name: str = None):
        ...
        if self.weapon.is_template:
            ...
            leveled_weapon.is_template = (
                False  # now it's a "unique" weapon (not a template one)
            )
        else:
        ...
        if stat in ["dmg", "damage"]:
            ...
            History.objects.create(
                campaign=self.campaign,
                author="SYSTEM",
                text=f"{self.name} leveled up his weapon's damage!",
            ).save()
        ...
        # else, you only get one more point
            ...
            if leveled_weapon.is_ranged:
                ...
                if leveled_weapon.range_level_points >= 5:
                    ...
                    History.objects.create(
                        campaign=self.campaign,
                        author="SYSTEM",
                        text=f"{self.name} leveled up his weapon's range!",
                    ).save()
                else:
                    History.objects.create(
                        campaign=self.campaign,
                        author="SYSTEM",
                        text=f"You obtained 1 range level point. Obtain {5-leveled_weapon.range_level_points} more to level up the weapon's range.",
                    ).save()
                ...
                History.objects.create(
                    campaign=self.campaign,
                    author="SYSTEM",
                    text=f"{self.name} tried to level up the range of a melee weapon, but that's not possible. The damage was leveled up instead.",
                )
            ...
            History.objects.create(
                campaign=self.campaign,
                author="SYSTEM",
                text=f"{self.name} tried to level up the weapon, but the stat was not valid. The damage was leveled up instead.",
            )
        ...
        if new_name:
            ...
            History.objects.create(
                campaign=self.campaign,
                author="SYSTEM",
                text=f"{self.name}'s weapon renamed to {new_name}",
            )

        leveled_weapon.save()
        self.weapon = (
            leveled_weapon  # now the character has the leveled up weapon equipped
        )
        self.level += 1  # the level increaces one point
        self.exp -= self.exp_top  # the experience reduces the top passed
        self.exp_top += int(self.exp_top * 0.2)  # the top increaces by a function
    ...
    def save(self, *args, **kwargs):
        if not self.character_class:
            self.character_class = (
                self.character_race if self.character_race != "Human" else "Warrior"
            )

        if not self.icon and not self.image:
            self.icon = self.get_default_entity_icon(
                self.character_race, self.character_class
            )
        ...
        if not self.physical_description:
            self.physical_description = (
                self.character_class + self.character_race
                if self.character_race != self.character_class
    ...
    def __str__(self):
        ...
        raceclass = (
            f"{self.character_race} {self.character_class}"
            if self.character_race != self.character_class
            else self.character_class
        )
...
class Monster(Entity, models.Model):
    ...
    def get_characters_in_range(self):
        ...
        for character in Character.objects.filter(campaign_id=self.campaign.id):
            if (
                abs(self.x - character.x) <= self.weapon.range
                and abs(self.y - character.y) <= self.weapon.range
            ):
    ...
    def save(self, *args, **kwargs):
        ...
        if not self.icon:
            self.icon = self.get_default_entity_icon(
                self.monster_race, self.monster_class
            )
        ...
        if not self.physical_description:
            self.physical_description = (
                self.monster_class + self.monster_race
                if self.monster_race != self.monster_class
                else self.monster_class
            )
    ...
    def __str__(self):
        raceclass = (
            f"{self.monster_race} {self.monster_class}"
            if self.monster_race != self.monster_class
            else self.monster_class
        )
...
class Treasure(models.Model):
    ...
    weapon = models.ForeignKey(
        Weapon, on_delete=models.CASCADE, default=None, null=True, blank=True
    )
    ...
    def discover(self):
        ...
        if self.treasure_type == "Weapon":
            try:
                ...
                self.icon = self.get_default_treasure_icon(
                    self.treasure_type, discovered=False
                )
        else:
            self.icon = self.get_default_treasure_icon(
                self.treasure_type, discovered=True
            )
        self.save()
    ...
    def save(self, *args, **kwargs):
        if not self.icon:
            ...
            if self.treasure_type == "Weapon" and self.discovered:
                ...
                self.icon = self.get_default_treasure_icon(
                    self.treasure_type, discovered=self.discovered
                )

        if (
            len(self.get_inventory().keys()) == 0
            and self.treasure_type != "Weapon"
            and self.discovered
        ):
            self.delete()

        if self.weapon is None and self.treasure_type == "Weapon":
            self.weapon = get_default_weapon(
                entity_class=DEFAULT_WEAPON_PER_CLASS.keys()[
                    randint(0, len(DEFAULT_WEAPON_PER_CLASS.keys()))
                ]
            )
            self.save()
...
class Tile(models.Model):
    ...
    def __str__(self):
        return (
            f"{{{self.campaign.id}}} [{self.id}] ({self.x},{self.y}) {self.tile_type}"
        )
