    ...
    if request.method == "POST":
        if "create_campaign" in request.POST:
            ...
            campaign = Campaign.objects.create(
                name=name, initial_story=initial_story, size_x=size_x, size_y=size_y
            )
            campaign.save()
...
def playerSelection(request):
    ...
    if request.method == "POST":
        ...
        campaign_id = (
            ...
            or Campaign.objects.filter(is_completed=False).first().id
        )
        ...
        if "create_player" in request.POST:
            ...
            character_class = (
                request.POST.get("class") or character_race
            )  # we use the race twice, if no class is selected
            ...
            # stats
            max_health = int(
                request.POST.get("max_health", int(request.POST.get("vitality")) * 10)
            )
            ...
            new_player = Character.objects.create(
                ...
                inventory=str(
                    {"gold": 20, "health potion": 5, "go back bone": 3, "key": 3}
                ),
                story=story,
            ...
            History.objects.create(
                campaign_id=campaign_id,
                author="SYSTEM",
                text=f"{new_player.name} has joined the party!",
            ).save()
            """
    ...
    players = Character.objects.filter(
        campaign_id=campaign_id, is_playable=True
    ).order_by("-level")

    return render(
        request,
        "playerselection.html",
        {"players": players, "campaign_id": campaign_id},
    )
...
def playerCreation(request):
    ...
    for weapon_name in weapon_names:
        default_weapon = Weapon.objects.filter(
            is_template=True, name=weapon_name
        ).first()
        if default_weapon:
    ...
    if request.method == "POST":
        ...
        if "prompt" in request.POST:
            ...
            # If the prompt is not empty, it's considered a message that needs to be sent to the AI to become a command
                ...
                History.objects.create(
                    campaign_id=campaign_id,
                    author=request.POST["player_name"],
                    text=prompt,
                    color="blue",
                ).save()
        ...
        target_id = (
            None if "target_id" not in request.POST else request.POST.get("target_id")
        )
        ...
        map_script, map_div = create_map(
            player, characters, monsters, treasures, tiles, target, host
        )
