...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        if result:
            self.stdout.write(
                self.style.SUCCESS(
                    f"Successfully created a {campaign.size_x}x{campaign.size_y} map for the campaign [{campaign_id}]: {campaign.name}"
                )
            )
        else:
            self.stdout.write(
                self.style.ERROR(
                    f"An error occurred while generating the map for the campaign [{campaign_id}]: {campaign.name}"
                )
            )
