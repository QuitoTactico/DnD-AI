...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        npc_counter = Character.objects.filter(
            campaign_id=campaign_id, is_playable=False
        ).delete()[0]

        # Elimina todos los monstruos que no son bosses clave y que coinciden con campaign_id
        monsters_counter = Monster.objects.filter(
            campaign_id=campaign_id, is_boss=False, is_key=False
        ).delete()[0]

        # Elimina todos los monstruos clave que no son bosses y que coinciden con campaign_id
        key_monsters_counter = Monster.objects.filter(
            campaign_id=campaign_id, is_boss=False, is_key=True
        ).delete()[0]

        # Elimina todos los jefes no clave que coinciden con campaign_id
        optional_bosses_counter = Monster.objects.filter(
            campaign_id=campaign_id, is_boss=True, is_key=False
        ).delete()[0]

        self.stdout.write(
            self.style.SUCCESS(
                f"Successfully deleted the map of the campaign [{campaign_id}]: {campaign.name}"
            )
        )
