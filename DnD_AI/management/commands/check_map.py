        ...
        try:
            ...
            for tile in tiles:
                ...
                if position in tile_positions:
                    self.stdout.write(
                        self.style.ERROR(
                            f"Duplicate tile found at position ({tile.x}, {tile.y})"
                        )
                    )
                    break
                else:
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f"Successfully checked a {campaign.size_x}x{campaign.size_y} map for the campaign [{campaign_id}]: {campaign.name}"
                )
            )
        except:
            self.stdout.write(
                self.style.ERROR(
                    f"An error occurred while checking the map for the campaign [{campaign_id}]: {campaign.name}"
                )
            )
