    ...
    def handle(self, *args, **kwargs):
        ...
        for i in range(min(x_init, x_final), max(x_init, x_final)):
            for j in range(min(y_init, y_final), max(y_init, y_final)):
                tile, created = Tile.objects.get_or_create(x=i, y=j)
                tile.tile_type = tile_type
                tile.save()
    
        self.stdout.write(self.style.SUCCESS(f'Successfully created ({tile_type}) tiles from ({x_init},{y_init}) to ({x_final},{y_final })'))
