...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        print(
            "Possible tile types: grass, dirt, path, dungeon, boss, god, psycho, hell"
        )
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"Successfully created ({tile_type}) tiles from ({x_init},{y_init}) to ({x_final},{y_final }) in the campaign {campaign_id}: {campaign.name}"
            )
        )
